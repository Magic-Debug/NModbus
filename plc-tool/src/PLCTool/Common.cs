...
namespace PLCTool
{
    public class Common
    {
        ...
        static ILogger<Common>? Logger => Program.Services.GetService<ILogger<Common>>();
        ...
        {
            CreateConfigFolder();
            string configfile = Path.Combine(Application.StartupPath, "PLCTool.dll.config");
        ...
        public void WriteLog()
        {
            while (true)
            {
                if (TcpCommunication.GetInstance().Connected)
                {
